...
class ActionManager {
  ...
  constructor(actionUrl, rowActionUrl, appendQueryParams, onSuccess, onError) {
    ...
    // get action-log iframe
    this.actionLog = document.getElementById("action-log");
    if (this.actionLog === null) {
      alert("action-log iframe not found!");
    }
    // save action-log base url and replace '/null' with ''
    this.actionLogBaseUrl = this.actionLog.src.replace("/null", "");
    // action-key
    this.actionKey = null;
  }
  ...
  submitAction(actionName, form, customResponse, isRowAction, element) {
    ...
    // generate action-key
    this.actionKey = window.crypto.randomUUID();

    let baseUrl = isRowAction ? this.rowActionUrl : this.actionUrl;
    let query = new URLSearchParams();
    query.append("name", actionName);
    // append action-key to query
    query.append("action-key", this.actionKey);
    ...
    if (customResponse) {
      ...
      // set action-log src
      this.actionLog.src = new URL(this.actionLogBaseUrl + "/" + this.actionKey);
